...
const line = (a) => {
  ...
  return `- **[${a.title}](${a.path})**${st} — ${a.summary}${tags}${auth}${by}${upd}\n`;
};

// per-zone indexes: <zone>/INDEX.md (agent opens only the zone it needs)
for (const top of zones) {
  let z = `# INDEX — ${top}\n\n`;
  z += `> Generated by \`scripts/reindex.mjs\`. Do not edit by hand. See the root \`INDEX.md\` for the map.\n\n`;
  z += `${byTop[top].length} article(s) · last rebuild: ${stamp}\n\n`;
  for (const a of byTop[top].sort((x, y) => x.path.localeCompare(y.path))) z += line(a);
...
idx += `> large base, grep titles/summaries/tags across the zones instead of loading everything.\n\n`;
idx += `Articles: **${articles.length}** · last rebuild: ${stamp}\n`;
...
if (articles.length <= INLINE_THRESHOLD) {
  for (const top of zones) {
    idx += `\n## ${top}\n\n`;
